...
@router.get("/find")
def get_all_warehouses(page: int = 1, size: int = 10, type: TypeWarehouse = None, db: Session = Depends(getDatabase)):
...
def update_warehouse(warehouse_id: int, newWarehouse: UpdateWarehouse, db: Session = Depends(getDatabase)):
    return WarehouseController.updateWarehouse(warehouse_id=warehouse_id, newWarehouse=newWarehouse, db=db)
