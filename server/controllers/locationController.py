        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(
                    ...
                    detail=f"Failed to fetch data from API: {response.text}",
                )
...
class LocationController:
    def getAllLocation(
        page: int = 1, size: int = 10, db: Session = Depends(getDatabase)
    ):
        skip = (page - 1) * size
        ...
        for item in locations:
            location = {
                ...
                "division": {"id": item.division_id, "name": item.division_name},
            }
    ...
    def getLocationById(location_id: int, db: Session = Depends(getDatabase)):
        ...
        result = {
            ...
            "ward": {"id": location_info.ward_id, "name": location_info.ward_name},
            "district": {
    ...
    def getLocationByWardId(ward_id: int, db: Session = Depends(getDatabase)):
        return db.query(LocationModel).filter(LocationModel.ward_id == ward_id).first()

    def createLocation(
        ward_id: int, location: CreateLocation, db: Session = Depends(getDatabase)
    ):
        db_location = LocationModel(ward_id=ward_id, address=location.address)
        db.add(db_location)
        db.commit()
            ...
            with db.begin():
                for division in data:
                    db_division = DivisionModel(
                        name=division["name"], type=division["division_type"]
                    )
                    db.add(db_division)
                    db.flush()
                    count_district_id = 0
