...
const AddressForm = ({ isOpen, onClose }: { isOpen: any, onClose: any }) => {
    ...
    const handleDistrictChange = (e: { target: { value: any; }; }) => {
        ...
        const selectedCityId = (document.getElementById('city') as HTMLInputElement).value;
        const selectedCity = cities.find(city => city.Id === selectedCityId);

        const selectedDistrict = selectedCity?.Districts.find((district: { Id: any; }) => district.Id === selectedDistrictId);
        ...
        <>
            {isOpen && (
                <div className="bg-white flex justify-center items-start p-2 space-x-5 h-20">
                    ...
                    <select className="border" id="district" onChange={handleDistrictChange}>
